...
#
# Vendored VERBATIM from https://github.com/rtk-ai/rtk (.claude/hooks/rtk-rewrite.sh)
# instead of running `rtk init -g`, which would write into Nix-managed ~/.claude.
# Bump alongside pkgs/rtk.nix when `rtk-hook-version` changes upstream.
#
